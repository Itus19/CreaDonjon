...
- [ ] **Un jet lancé depuis la fiche répond à la demande en cours** (V3-B5) — **V3-B5 n'existe pas encore** (lot B, toujours ouvert). Cette colonne appelle donc directement `/api/entities/:id/actions/*`, exactement comme le fait la fiche large aujourd'hui : un jet compte pour de vrai, mais rien ne le rattache à une demande posée par le moteur. À reprendre quand V3-B5 sera fait — pas une régression introduite ici, le même chemin que la fiche existante.
