...
### V3-B5 — Le moteur demande un jet, il ne le lance pas · `L` — **Phase 1 faite le 24 septembre**

*Ouvert le 22 septembre, en dessinant le lot D. **Ce ticket corrige la dernière étape de V3-B1** : il ne la jette pas.*

**Découpé en deux phases, avec l'accord de l'auteur (24 septembre : « ta méthode me va très bien »).** Le ticket mélangeait un mécanisme central (poser/encaisser, persister, chaîner, journaliser) et son branchement dans des composants **partagés** avec des contextes hors solo (écran Initiative, fiche large, campagne à MJ humain) — les boutons de la fiche jouable et le volet de dés. Rendre ces composants conscients d'une demande était le risque le plus large du ticket ; la Phase 1 livre le mécanisme et le seul chemin qui n'en dépend pas (« annoncé à la main », dans `IntentBar`), la Phase 2 reste à ouvrir pour le reste.
...
- [ ] **Reporté à la Phase 2.** Les boutons de la fiche et le volet de dés **savent qu'une demande attend** et s'y rattachent. C'est le critère de D5 (« un seul chemin vers la résolution, jamais deux ») porté côté serveur — ces composants sont **partagés** avec des contextes hors solo (Initiative, fiche large, MJ humain), où une demande n'existe pas ; les y rendre conscients d'un `pending_request` est son propre chantier, pas étendu ici sans y avoir réfléchi.
- [x] Un dé annoncé à la main arrive **nu** : le moteur ajoute le modificateur figé à la pose (`resolveIntentRequest`), jamais le client — `IntentBar.tsx` envoie seulement le naturel et `origin: "a_la_main"`. Le `session_event`/`dice_rolls.detail` porte cette origine. **L'outil de dés (volet) n'envoie pas encore ce naturel** — Phase 2, même raison que ci-dessus.
...
**Vérifié en intégration, base réelle** (`turnIntent.pendingRequest.integration.test.ts`, 7 cas) : pose sans rien lancer ; encaissement au total figé (naturel + modificateur de la pose, jamais recalculé) ; refus sans demande en attente ; refus hors bornes (structurel puis contre `die_max`) ; abandon journalisé au remplacement ; chaîne attaque→dégâts avec plafond correct ; doublement du plafond au critique, modificateur inchangé.

**Pourquoi `L` et pas `M`** : la demande introduit un état de jeu de plus, donc une forme à décider, un endroit où l'écrire et un cycle de vie (posée, honorée, abandonnée). C'est moins du code que des décisions — et la Phase 2 (fiche, volet de dés) reste ouverte pour cette raison précise : la connexion à des composants partagés est elle-même une décision, pas juste du câblage.
...
- [ ] **Un jet lancé depuis la fiche répond à la demande en cours** (V3-B5) — **la Phase 1 de V3-B5 est faite, la Phase 2 (boutons de fiche et volet de dés conscients d'une demande) reste ouverte**, avec l'accord explicite de l'auteur (24 septembre) pour la reporter. Cette colonne appelle donc toujours directement `/api/entities/:id/actions/*`, exactement comme le fait la fiche large aujourd'hui : un jet compte pour de vrai, mais rien ne le rattache à une demande posée par le moteur. À reprendre quand la Phase 2 sera faite — pas une régression introduite ici, le même chemin que la fiche existante.
