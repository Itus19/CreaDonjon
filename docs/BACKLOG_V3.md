...
**Défaut relevé en passant, non corrigé** : `resolveOneEntryBlocks` (`rules.ts:309`) fait confiance au `block_type` que porte une surcharge. Une surcharge `add_block` mal formée — sans `block_type`, ce qui est arrivé à mon premier essai — fait tomber la résolution de TOUTE la fiche sur un `Cannot read properties of undefined`, au lieu d'ignorer le bloc fautif en le signalant. Sans conséquence tant que seul le code écrit les surcharges ; à traiter avant V3-A5, qui ouvrira cette écriture à l'auteur.
