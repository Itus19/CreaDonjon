...
- [ ] **Les caractéristiques tiennent en deux rangées au plus.** Le pavé d'origine — six cartes bordées de trois lignes — mesure **144 px** pour six nombres, et c'est le plus gros poste de la colonne après les onglets. Trois refontes ont été dessinées et comparées dans l'esquisse ; **l'auteur a finalement retenu la grille** le 23 septembre, complétée du score. L'interrupteur de comparaison et les deux autres variantes sont partis avec le choix :
...
- [ ] **Le Sac s'ouvre sur la bourse et la charge, sur une seule ligne** — les deux choses qu'on vient y vérifier en jouant. La charge est **la jauge circulaire des PV et du niveau**, pas une barre : une seule forme pour « une part d'un tout » dans toute la colonne, sinon c'est deux fois à apprendre. Les pièces occupent la largeur qu'elle libère, alignées sur son axe. Cette jauge-là est **plus petite que celles du bandeau** (44 px contre 48), **sans légende dessous**, et porte tout son rapport **en fraction à l'intérieur** : le poids au-dessus du trait, la capacité en dessous. La police ne descend pas — `text-xs` est le plancher de la charte — donc ce qui rentre se règle par la géométrie : deux lignes de 12 px mesurent 25 px de haut, et à cette hauteur un anneau de 40 px n'offre plus que 12,6 px de large. D'où 44 px, et des nombres courts : **le poids est arrondi au kilo pour l'affichage** (`23,5` mesure 25 px, `24` en mesure 14), la valeur exacte restant dans l'infobulle — un demi-kilo ne change aucune décision, un texte qui déborde de son anneau, si. Mesuré : coin de texte le plus éloigné à 14,5 px pour un rayon intérieur de 15,4. Le clic donne le pourcentage comme ailleurs. La jauge passe au ton d'alerte dès que `palier` n'est plus `none`. On y équipe un objet ; dans Magie, on y prépare un sort.
- [ ] **Tout ce qui se lance se clique** : modificateur de caractéristique (un test), sauvegarde, compétence, attaque, dégâts.
- [ ] **Actions et Magie reprennent la présentation de `ActionsTab` et `MagicTab`**, pas une troisième façon de montrer les mêmes choses. Les boutons de jet sont `ActionButton` **importé**, jamais recopié : « la même esthétique » ne se vérifie pas à l'œil quand on la réécrit. Les couleurs d'école viennent de `MAGIC_SCHOOL_COLOR_VAR`.
- [ ] **Les deux boutons d'une action sont sur la même ligne** (auteur, 23 septembre) — attaque et dégâts pour une arme, attaque et lancement pour un sort.
- [ ] **L'onglet Actions ne tient aucune liste à lui.** Il dérive ce qu'il montre : une arme **équipée** dans le Sac y pose son attaque, **un sort préparé dans Magie y pose son lancement** — le second est le mécanisme du premier, appliqué aux sorts (auteur, 23 septembre). Un sort mineur y est toujours, comme dans la vraie fiche.
- [ ] **Les emplacements de tous les niveaux disponibles sont lançables** : un sélecteur choisit lequel dépenser, et ne propose que des niveaux **au moins égaux** à celui du sort — on surclasse vers le haut, jamais vers le bas. En tête de l'onglet, les emplacements en pastilles pleines ou creuses, un niveau sans emplacement n'apparaissant pas.
- [ ] **L'onglet Magie ne montre PAS les emplacements** (auteur, 23 septembre) : ils se dépensent dans Actions, où l'on lance. Magie montre ce qu'on vient y faire — **combien de sorts sont préparés sur combien la classe en autorise**, mêmes pastilles que les emplacements parce que « il m'en reste combien » est la même question.
- [ ] **La limite est tenue, et elle vient des règles.** Le nombre de sorts préparables est une valeur de la table de la classe (ruleset), jamais un calcul d'écran. Au-delà, le bandeau « Préparer » devient inactif et dit pourquoi — la limite se tient au clic, pas à l'enregistrement : un bouton qui accepte puis refuse est un bouton qui ment. **Se dé-préparer reste toujours possible**, sinon on s'enferme à la limite atteinte.
- [ ] **Les sorts mineurs ne comptent pas dans la limite** : toujours prêts, leur bandeau dit « Mineur » au lieu d'offrir un bouton qui ne ferait rien.
- [ ] **Le décompte d'emplacements appartient au serveur.** L'esquisse le tient en état local pour montrer le geste ; l'écran, lui, ne décidera d'aucun résultat de règle (règle absolue 8).
