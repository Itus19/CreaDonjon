...
- [x] Budget d'entrée : moins de 600 tokens par tour, mesuré et journalisé dans `ai_usage_log` — **333 tokens d'entrée**, mesuré sur un vrai appel (voir tableau plus bas), largement sous la barre.

**La narration est câblée (24 septembre).** `src/server/ai/soloNarration.ts` (`narrateSoloTurn`) appelle `AiProvider` avec le contexte de V3-B3 (`buildSoloTurnContext`) et l'enum de PNJ fermé de `src/core/ai/soloNarrationProposal.ts` (généralise `spikeSoloProposal.ts` à des PNJ réels, plus des identifiants figés) ; un événement `narration` de plus part dans `session_events`, porté par `payload.from_event` — le même identifiant que `rule_application` utilise déjà pour se rattacher au jet. `app/api/solo/tour/route.ts` l'appelle **après** `playTurn`, en best-effort strict (`try/catch` muet) : un fournisseur injoignable, une sortie invalide ou une limite de débit ne retire rien au tour, déjà complet et journalisé — `specs/cible-locale-et-ia.md` §4 appliqué au tour solo.

**Le garde-fou de V3-B1 est réécrit, pas supprimé.** `turnIntent.noAi.test.ts` annonçait lui-même « le jour où V3-B2 branchera la narration, ce test tombera ». C'est fait consciemment : les quatre fichiers qui font tourner la mécanique (`intent.ts`, `turnIntent.ts`, `turnLoop.ts`, `IntentBar.tsx`) restent interdits de tout appel d'IA, sans exception. `route.ts` en sort de cette liste — il enveloppe la mécanique, il ne la remplace pas — et gagne son propre test : `getOpenAiCompatibleProviderFromEnv(` et `narrateSoloTurn(` doivent apparaître, dans le texte source, **après** l'appel à `playTurn(`. Un ordre vérifié, plutôt qu'une absence.

**Le bouton « raconter ce tour » n'a pas d'écran où vivre.** `narrateSoloTurn` est déjà rejouable pour un tour passé — elle ne relit ni ne relance aucun dé, `facts`/`changes`/`hints`/`fromEventId` viennent toujours de l'appelant. Il ne manque qu'un bouton, et la colonne qui l'afficherait (le fil, V3-D4) n'existe pas encore. Prêt pour qui la construira, même motif que V3-B3/B5 laissés prêts pour un appelant à venir.

**Infrastructure : le modèle tourne sur une autre machine, et c'est désormais la cible retenue.** `AI_LOCAL_BASE_URL`/`AI_LOCAL_MODEL` étaient vides sur le portable depuis le 22 septembre. Réglé le 24 : LM Studio tourne sur le poste fixe (`192.168.1.106:1234`), joint depuis le portable sur le réseau local — modèle retenu, `gemma-4-e4b-uncensored-hauhaucs-aggressive`. Deux réglages ont été nécessaires côté fixe, ni l'un ni l'autre par défaut : la règle de pare-feu Windows pour le port 1234, et l'option « Serve on Local Network » de LM Studio (sans elle, le serveur n'écoutait que `127.0.0.1` — aucune règle de pare-feu n'y change rien, rien n'écoutait le réseau du tout). C'est la cible que l'auteur visait explicitement (« jouer depuis le canapé, profiter de la puissance du fixe ») — `AiProvider` n'a jamais supposé que le modèle tourne sur la même machine que le serveur, la configuration seule suffisait.

**Vérifié en direct le 24 septembre, avec le vrai modèle**, sur `ClaudeLand`/Fine Lââm (traces dans `session_events`, journal en ajout seul — jamais supprimées, à la différence d'une fiche de contrôle : conforme à la conception même de ce journal) :

| Ce qui a été exercé | Résultat |
|---|---|
| « je regarde autour de moi, dans la salle commune » (action libre, hors scène posée) | Narration reçue : *« Sous le soleil éclatant de cette matinée, vous êtes dans la salle commune à huit heures précises. Fine Lââm balaie du regard l'espace autour de lui... »* |
| `ai_usage_log` | `purpose: solo_turn`, `input_tokens: 333`, `output_tokens: 502`, modèle correctement journalisé |
| `session_events` | Un `narration` de plus, `actor: "ai"`, `payload.from_event` pointant exactement vers l'id du `player_action` qui l'a précédé |
| Le tour sans narration | Toujours valide : le `try/catch` best-effort ne l'a jamais mis en jeu pendant les vérifications précédentes, fournisseur éteint |

**À écrire sur la machine où le modèle tourne — désormais vrai pour le fixe, plus pour le portable.** S1 comme S2 avaient montré que c'est la prose qu'il faut voir pour la juger ; c'est fait. Le reste de la boucle reste vérifiable fournisseur éteint, ce qui demeure la meilleure démonstration du critère transverse « le solo doit rester jouable sans IA du tout ».
...
**Découpé en deux phases, avec l'accord de l'auteur (24 septembre : « ta méthode me va très bien »).** Le ticket mélangeait un mécanisme central (poser/encaisser, persister, chaîner, journaliser) et son branchement dans des composants **partagés** avec des contextes hors solo (écran Initiative, fiche large, campagne à MJ humain) — les boutons de la fiche jouable et le volet de dés. Rendre ces composants conscients d'une demande était le risque le plus large du ticket ; la Phase 1 a livré le mécanisme et le seul chemin qui n'en dépendait pas (« annoncé à la main », dans `IntentBar`). **La Phase 2, faite le même jour après coup, branche les boutons de la fiche** — en restant, elle aussi, hors de tout composant partagé (voir plus bas) ; seul le volet de dés freeform reste délibérément hors-jeu.
...
- [x] `executeIntent` se scinde : **proposer une demande** (`proposeIntentRequest`) d'un côté, **encaisser un résultat** (`resolveIntentRequest`) de l'autre — `src/server/services/turnIntent.ts`. L'interprétation, le catalogue, la correction et le journal de B1 ne changent pas ; `executeIntent` lui-même est intact, toujours le chemin de l'action libre.
- [x] La demande est **persistée** : `entity_runtime_state.pending_request` (jsonb validé par Zod, aucune migration SQL — même précédent que `budgets`/`inspiration`). Elle survit à un rechargement de la même façon que le reste de l'état de jeu.
- [x] Au plus une demande en cours par personnage. Une nouvelle intention **remplace** la précédente (`mergeRuntimeState`, jamais un empilement) — et l'abandon est journalisé (`session_events`, kind `world_update`, note « Demande abandonnée : … — remplacée par : … »), jamais silencieux. Vérifié en intégration (base réelle).
- [x] **Phase 2.** Les boutons de la fiche **savent qu'une demande attend** et s'y rattachent — `FicheJouableSolo.tsx` lit `pending_request` directement dans ce que `/api/entities/:id/sheet` lui rend déjà (rien de plus à charger : c'est tout l'intérêt de l'avoir persisté en Phase 1). Quand un clic correspond à la demande posée (même genre, même `action_id`), il l'encaisse au lieu de lancer un jet immédiat indépendant. **Le volet de dés (l'outil freeform) reste hors-jeu, délibérément** : il est partagé par toute la campagne, sans notion d'entité ni d'action liée — lui faire porter une demande précise serait un chantier à part, pas une extension naturelle de son rôle actuel (jets improvisés, MJ compris).
- [x] Un dé annoncé à la main arrive **nu** : le moteur ajoute le modificateur figé à la pose (`resolveIntentRequest`), jamais le client — `IntentBar.tsx` envoie seulement le naturel et `origin: "a_la_main"`. **Un bouton de fiche, lui, n'envoie aucun nombre** (`origin: "fiche"`) : c'est le SERVEUR qui tire, avec les mêmes fonctions que les anciens boutons d'action (`resolveCheckRoll`/`resolveDamageRoll`, `action.ts`) — un modificateur nul isole le naturel sans dupliquer la mécanique d'avantage ni le doublement des dés au critique. Le `session_event`/`dice_rolls.detail` porte la bonne origine dans les trois cas.
- [x] Un résultat annoncé est **borné et refusé au-delà**, avec un message — jamais écrêté en silence. **Corrigé en écrivant ce ticket : la borne n'est PAS 20 dans tous les cas.** Une demande de dégâts se lit sur le dé de l'ARME (`pending_request.die_max`, fixé à la pose depuis `weapon.damageDice`, doublé si le coup est critique — jamais le modificateur, même règle que `doubleDiceCounts` dans `action.ts`), un d20 seulement pour l'attaque, le test ou la sauvegarde. Vérifié en intégration : un dégât de cimeterre (1d6) refuse un naturel de 7, l'accepte jusqu'à 6, jusqu'à 12 en critique.
- [x] Un tour peut porter **plusieurs jets** : une attaque qui touche **chaîne** une demande de dégâts (posée automatiquement, même acteur, même cible) plutôt que de clore le tour — deux annonces consécutives du joueur, jamais un formulaire à deux nombres imposé. Les dégâts chaînés ne dépensent pas de budget et ne font pas avancer l'horloge une seconde fois : c'est la même action que l'attaque qui les a déclenchés (`applyResolvedTurn`, `turnLoop.ts`, paramètre `choiceKind: null`).
- [x] Les tests de V3-B1 restent verts. `turnIntent.noAi.test.ts` continue d'interdire tout appel de modèle sur `turnIntent.ts`/`turnLoop.ts`/`IntentBar.tsx`, inchangé par ce ticket.

**Bogue trouvé en écrivant ce ticket, corrigé au passage (`turnLoop.ts`).** `eventsForAttack` ne recevait jamais `isCritical` : le payload stocké porte le champ `critical`, mais le cast lisait `isCritical` — toujours `undefined` après coercition. Un coup critique ne déclenchait donc **aucune** règle accrochée à `attack_hit`/critique depuis que V3-B2 a câblé ces déclencheurs. Corrigé par une extraction explicite du bon nom de champ ; aucun test existant ne portait sur ce cas (silencieux, comme le sont les bogues de cast), donc rien n'a viré au rouge — seule la lecture attentive du code déjà en place, en construisant la boucle d'encaissement, l'a fait apparaître.

**Le déroulé côté écran (`IntentBar.tsx`), Phase 1.** « Lancer » sur un choix mécanique pose la demande (`POST /api/solo/tour` rend `{ pending }` au lieu d'un tour joué) ; l'écran affiche « Jet demandé : … », le modificateur figé, la CA/DD si connue, et un champ « Dé annoncé (1-N) » où N est `pending.die_max` — jamais un « 1-20 » écrit en dur. Valider appelle `POST /api/solo/tour/encaisser` ; une attaque qui touche renvoie `chained`, et l'écran repart aussitôt en attente du naturel de dégâts, même tour. L'action libre et la fiche hors campagne gardent leur aller simple d'avant ce ticket — rien à différer quand aucun jet n'est en jeu.

**Une limite Phase 1 assumée, pas un oubli.** `pending` est un état de composant : un rechargement de page le vide côté écran, bien que la demande survive côté serveur (`entity_runtime_state`). Relire la demande en cours au montage n'est pas construit — même limite déjà documentée pour le fil des tours en B1/B2, et le même chantier (D4, le fil) la résoudra probablement pour les deux à la fois.

**Le déroulé côté écran (`FicheJouableSolo.tsx`), Phase 2.** Un encart « Jet demandé : … » apparaît sous l'en-tête dès qu'une demande est posée (n'importe où — `IntentBar` ou un rechargement l'a laissée là). Les boutons d'attaque/dégâts (`ActionsTab`) et les tuiles de caractéristique/sauvegarde/compétence (`CaracteristiquesEtCompetences`) appellent `resolveFromFiche()` au lieu de leur chemin habituel dès qu'ils correspondent à la demande ; sinon, rien ne change — un clic hors demande reste un jet immédiat comme avant ce ticket. **Zéro changement à `ActionButton`, `ActionsTab.tsx`, `MagicTab.tsx` ni au contexte partagé `useDiceRoll`** (`DiceRollPanel.tsx`) : tout le branchement est local à `FicheJouableSolo.tsx`, qui décide lui-même, clic par clic, quel chemin emprunter — l'Initiative, la fiche large et les campagnes à MJ humain n'ont rien changé sous eux.

**Un trou trouvé en vérifiant en direct (24 septembre) : la fiche ne découvrait une demande QU'AU montage.** `IntentBar` (colonne centrale) et `FicheJouableSolo` (colonne droite) sont deux arbres React indépendants, sans canal entre eux — posé comme une limite acceptable en écrivant le code. Mais en jouant vraiment (poser une demande dans `IntentBar`, puis regarder la fiche), l'encart « Jet demandé » n'apparaissait jamais côté fiche : elle avait chargé `pending_request` une seule fois, avant la pose, et ne le relisait plus jamais. Ce n'était pas un cas bord rare — c'est le déroulé NORMAL, les deux colonnes étant ouvertes en même temps par construction de l'écran. **Corrigé** : `FicheJouableSolo.tsx` sonde `/api/entities/:id/sheet` toutes les 4 secondes au lieu d'un chargement unique — le coût d'un GET déjà bon marché, répété, plutôt qu'un canal Realtime dédié (`DiceRollPanel.tsx` en a un pour `dice_rolls`, mais c'est un chantier à part, pas étendu ici sans y avoir réfléchi). Vérifié en direct après correction : la fiche affiche la demande posée par `IntentBar` sans rechargement, un bouton qui y répond l'encaisse et retrouve son comportement normal juste après.

**Ce qui reste une staleness assumée, dans l'autre sens.** Si la fiche encaisse une demande qu'`IntentBar` affichait encore, son propre encart reste affiché jusqu'à son prochain sondage (`IntentBar` ne sonde rien lui-même — seul l'état SERVEUR compte, jamais une seconde source de vérité). Un « Annoncer » tapé sur cet encart périmé échoue proprement (« Aucun jet n'est demandé en ce moment »), jamais un double jet ni une corruption — asymétrique par rapport au trou ci-dessus, mais sans le même impact : la fiche est le chemin qui doit rester à jour pour que la Phase 2 serve à quelque chose, `IntentBar` peut se permettre un léger retard sur sa propre demande.

**Vérifié en intégration, base réelle** (`turnIntent.pendingRequest.integration.test.ts`, 10 cas) : pose sans rien lancer ; encaissement au total figé (naturel + modificateur de la pose, jamais recalculé) ; refus sans demande en attente ; refus hors bornes (structurel puis contre `die_max`) ; abandon journalisé au remplacement ; chaîne attaque→dégâts avec plafond correct ; doublement du plafond au critique, modificateur inchangé ; **Phase 2** — un bouton de fiche sans demande en attente refuse, un test de compétence tombe dans les bornes attendues, une attaque qui touche chaîne des dégâts bornés au VRAI dé de l'arme (jamais un maximum uniforme entre 1 et `die_max`, qui aurait donné une distribution fausse pour une arme à plusieurs dés).

**Vérifié en direct, le 24 septembre, sur la base réelle** (fiche jetable dans ClaudeLand, tout supprimé après — entité, blocs, appartenance de campagne, `dice_rolls`, `session_events`, état de jeu). Posé « je fais un test de discrétion » dans `IntentBar` → proposition « Test · Discrétion » → « Lancer » pose la demande, affichée dans les DEUX colonnes après la correction du sondage (voir ci-dessus). Cliqué « Discrétion » dans la fiche → le volet de dés s'ouvre avec le résultat, `dice_rolls.context.origin` vaut `"fiche"`, `pending_request` repasse à `null`. Un second clic sur le même bouton, la demande déjà honorée, relance normalement (chemin d'avant ce ticket, inchangé) — exactement le comportement voulu.

**Pourquoi `L` et pas `M`** : la demande introduit un état de jeu de plus, donc une forme à décider, un endroit où l'écrire et un cycle de vie (posée, honorée, abandonnée). C'est moins du code que des décisions — et la Phase 2 (fiche) l'a confirmé : la brancher a exigé de choisir, pour chaque bouton, s'il répond ou non à la demande en cours, et de refuser sciemment de brancher le volet de dés plutôt que de le forcer dans un rôle qui n'est pas le sien.
...
- [x] **Un rendu par type** : narration en prose ; action du joueur alignée à droite, discrète ; jet en encart compact **dont la trace se déplie au clic** ; application de règle repliable (dont ce qui n'a pas pu s'appliquer, voir bogue ci-dessous) ; changement du monde en mention discrète — **y compris les demandes posées/honorées/abandonnées de V3-B5** (`world_update`, même forme). **Réponse du MJ** : un liseré d'accent, « MJ — hors du temps de jeu », la question puis la réponse (`NoteRow`, `Fil.tsx`) — genre `note` de `session_events`, jusque-là jamais utilisé.
- [x] **Un seul champ de saisie pour ce que le joueur ÉCRIT et pour ce qu'il ANNONCE à la main.** `IntentBar.tsx` n'a plus de panneau « Jet demandé » séparé : le MÊME champ change de rôle — bordure à l'accent, texte temporaire qui devient la demande (« Sauvegarde de Dextérité DD 13 — lance depuis ta fiche, ou écris ton résultat »), et ce qu'on y tape devient le naturel annoncé plutôt qu'une phrase à interpréter. **Écart assumé par rapport à la lettre du ticket** : un jet fait depuis la fiche ou le volet de dés continue de résoudre DIRECTEMENT (V3-B5 Phase 2, déjà vérifié), plutôt que de s'inscrire en texte ici pour exiger un second clic — les deux colonnes sont des arbres React distincts, et faire remonter du texte de l'une vers l'autre aurait ajouté un pas là où B5 en retire déjà un. Le champ unique porte donc le seul chemin qui n'avait pas déjà de résolution directe.
- [x] Le champ **grandit avec son contenu**, d'une ligne à huit (mesuré sur `scrollHeight`, jamais un compte de frappes) ; les boutons restent dessous.
- [x] **Les jets attachés sont au pluriel** : une attaque qui touche chaîne une demande de dégâts (V3-B5) — le MÊME champ repart aussitôt en attente du second nombre, même geste, sans repasser par une phrase.
- [x] **Deux boutons, `Jouer` et `MJ`.** `Jouer` résout le tour, inchangé. `MJ` pose la même phrase comme une question hors du temps de jeu — **ne touche jamais au tour** : ni jet, ni `entity_runtime_state`, ni horloge de scène. Détection de question par liste **fermée** (`looksLikeQuestion`, `src/core/rules/intent.ts` ; marqueurs en français dans `GM_QUESTION_MARKERS_FR`, `src/i18n/fr.ts`, même discipline que `INTENT_VERBS_FR`) : un point d'interrogation en fin de phrase, ou une tournure interrogative en tête. Le bouton deviné passe devant (style primaire) — **jamais de reroutage silencieux** : les deux boutons restent cliquables quoi que la détection réponde, une ligne l'explique (« Ça ressemble à une question — MJ ne fait pas avancer le temps »).
  - **Architecture** : `askSoloGm` (`src/server/ai/soloGmQuestion.ts`, seule porte d'entrée IA de ce chemin, règle absolue 12) réutilise le contexte déjà bâti pour la narration de tour (scène, PNJ présents, quêtes, dernières narrations — `buildSoloGmQuestionContext`, `src/server/services/turnContext.ts`) sans `facts`/`changes`/`hints` puisqu'aucun tour n'a eu lieu. Réponse en texte libre (pas d'appel outillé, rien à contraindre) journalisée en `kind: "note"`. Route `app/api/solo/mj/route.ts` en **échec dur** (même style que `.../tour/[eventId]/autrement/route.ts`) : poser la question EST l'action entière, un échec ne doit jamais rester silencieux.
  - **Garde-fou testé, pas seulement documenté** : `turnIntent.noAi.test.ts` vérifie par lecture statique que ni la route ni `soloGmQuestion.ts` ne mentionnent une seule fonction de la boucle de tour (`playTurn`, `advanceTime`, `putSceneState`, etc.), qu'ils appellent bien `runAiCompletion(`, et qu'ils journalisent en `kind: "note"` — le même motif que le garde-fou « aucun appel IA avant `playTurn`» de V3-B1.
  - **Limite assumée, pas une omission** : aucune recherche dans le wiki ou le SRD pour une question de règle — ça demande la RAG de V3-E2, qui n'existe pas encore (lot E, ouvert seulement après trois séances jouées). Le modèle répond donc seul, avec la consigne explicite de dire qu'il ne sait pas plutôt que d'inventer.
- [ ] **Reporté, mais plus par dépendance : par absence de besoin.** La paire `Jouer`/`MJ` existe désormais ; la saisie vocale reste une commodité non demandée, sans coût en tokens ni urgence à l'ouvrir.
...
**`pending` change de propriétaire en Phase 2 : `SoloScreen.tsx`, plus `IntentBar.tsx`.** Une demande peut être honorée depuis la fiche (colonne voisine) sans que la barre de saisie en soit informée autrement qu'en le sondant — et `SoloScreen.tsx` sondait déjà le fil au même rythme. Un seul minuteur relit désormais le fil ET l'état de jeu (`entity_runtime_state.pending_request`) à chaque tick, `IntentBar` reçoit `pending` en prop et prévient son parent après ses propres actions (`onActed`) pour un rafraîchissement immédiat plutôt que d'attendre le sondage — même raison que le sondage de `FicheJouableSolo.tsx` en Phase 2 de V3-B5. Ceci referme, pour le champ de saisie, la même staleness inter-colonnes déjà documentée pour la Phase 1 de ce ticket.

**Vérifié en direct, le 24 septembre, sur la base réelle** (deux fiches jetables successives dans ClaudeLand, tout supprimé après — entité, blocs, appartenance de campagne, jets, journal, à chaque fois). **Phase 1** : le fil affiche une action de joueur passée, sa narration, une demande posée, le jet résolu depuis la fiche avec son origine, et l'abandon de la demande — sans rechargement. **Phase 2** : posé « je fais un test de discrétion » dans le champ unique → proposition « Test · Discrétion » → « Lancer » fait passer le MÊME champ en mode demande (bordure à l'accent, placeholder « Discrétion — test de Dextérité — lance depuis ta fiche, ou écris ton résultat (1-20) ») → tapé « 14 » → « Annoncer » encaisse, le champ redevient normal, la fiche efface son propre encart, et le fil montre le jet (origine « annoncé ») puis « Demande honorée ».

**Une instabilité d'outil rencontrée en vérifiant, sans rapport avec le code.** À deux reprises, l'onglet du navigateur de vérification est resté visuellement figé sur un rendu React suspendu (`<div hidden>` jamais révélé) malgré des réponses serveur correctes, et une salve de vingt-trois requêtes identiques s'est produite d'un coup — cohérent avec un onglet mis en arrière-plan dont les minuteurs sont throttlés puis rattrapés d'un coup par le navigateur, pas avec une boucle de rendu : dans les deux cas, un nouvel onglet ou quelques secondes d'attente ont suffi à voir l'état correct s'afficher, et la base de données confirmait déjà le bon résultat pendant l'attente. Une troisième forme du même genre d'instabilité, un 404 authentique sur `/joueur/solo` pendant qu'une route sœur répondait 200 sur la même instance de serveur, s'est résolue par un cache Turbopack vidé (`.next/cache`) et un redémarrage — pas par un changement de code.
...
- [x] **Six onglets ne tiennent pas dans 300 px** — le dernier se coupe. Les compétences vivent donc sous les caractéristiques, repliées, là où la vraie fiche les met : elles n'y sont pas un onglet non plus.
- [x] **Les caractéristiques tiennent en deux rangées au plus.** Le pavé d'origine — six cartes bordées de trois lignes — mesure **144 px** pour six nombres, et c'est le plus gros poste de la colonne après les onglets. Trois refontes ont été dessinées et comparées dans l'esquisse ; **l'auteur a finalement retenu la grille** le 23 septembre, complétée du score. L'interrupteur de comparaison et les deux autres variantes sont partis avec le choix :
...
| **Barrette** — une seule rangée de six | **~86 px** | Quatre étages par colonne dans un encadré chacun. Écartée : six colonnes de 40 px serrent le nombre, et la rangée pousse les onglets vers le bas |
...
- [x] **Une jauge à zéro est vide.** Un `strokeLinecap="round"` dessine un point même sur une valeur nulle, et un point d'épuisement à zéro se lit comme un début d'épuisement.
- [x] **Chaque anneau porte son chiffre au centre, et un clic sur ce chiffre le bascule en pourcentage** (auteur, 23 septembre). Chaque anneau garde son propre affichage : « combien de PV me reste-t-il » et « où j'en suis du niveau » ne se lisent pas de la même façon, l'une en valeur, l'autre en proportion. Le chiffre est le bouton, et sa cible fait 40 px — la règle des 24 px de V2.1-27 vaut ici comme ailleurs.
- [x] **L'épuisement se lit sur 6**, pas sur un maximum inventé : c'est la borne de `zRuntimeState` (`.min(0).max(6)`), parce que le niveau 6 est la mort. Son anneau se remplit à l'envers des deux autres — il monte quand ça va mal — d'où le ton d'alerte.
...
- [x] **Le Sac s'ouvre sur la bourse et la charge, sur une seule ligne** — les deux choses qu'on vient y vérifier en jouant. La charge est **la jauge circulaire des PV et du niveau**, pas une barre : une seule forme pour « une part d'un tout » dans toute la colonne, sinon c'est deux fois à apprendre. Les pièces occupent la largeur qu'elle libère, alignées sur son axe. Cette jauge-là est **plus petite que celles du bandeau** (44 px contre 48), **sans légende dessous**, et porte tout son rapport **en fraction à l'intérieur** : le poids au-dessus du trait, la capacité en dessous. La police ne descend pas — `text-xs` est le plancher de la charte — donc ce qui rentre se règle par la géométrie : deux lignes de 12 px mesurent 25 px de haut, et à cette hauteur un anneau de 40 px n'offre plus que 12,6 px de large. D'où 44 px, et des nombres courts : **le poids est arrondi au kilo pour l'affichage** (`23,5` mesure 25 px, `24` en mesure 14), la valeur exacte restant dans l'infobulle — un demi-kilo ne change aucune décision, un texte qui déborde de son anneau, si. Mesuré : coin de texte le plus éloigné à 14,5 px pour un rayon intérieur de 15,4. Le clic donne le pourcentage comme ailleurs. La jauge passe au ton d'alerte dès que `palier` n'est plus `none`. On y équipe un objet ; dans Magie, on y prépare un sort.
- [x] **Tout ce qui se lance se clique** : modificateur de caractéristique (un test), sauvegarde, compétence, attaque, dégâts.
- [x] **Actions et Magie reprennent la présentation de `ActionsTab` et `MagicTab`**, pas une troisième façon de montrer les mêmes choses — les composants réels, **importés sans une ligne recopiée**, pas une réécriture qui « ressemble ». Les boutons de jet sont `ActionButton` **importé**, jamais recopié. Les couleurs d'école viennent de `MAGIC_SCHOOL_COLOR_VAR`.
- [ ] **Les deux boutons d'une action sont sur la même ligne — non tenu, et pour une raison précise.** `ActionButton` (`InventoryPanel.tsx`) est posé dans une grille `grid-cols-[repeat(auto-fit,minmax(15rem,1fr))]` : sous ~480 px de large, `auto-fit` replie ses deux boutons l'un sous l'autre. La colonne solo fait 280 à 400 px — trop étroite pour deux colonnes de 15 rem. Faire tenir ce critère exigerait de baisser ce seuil dans `ActionsTab.tsx`, donc de changer l'affichage de la fiche **large** aussi (le composant est partagé, réutilisé tel quel par le bullet précédent) — un arbitrage de largeur qui n'est pas à ce ticket de trancher seul. Les deux boutons restent donc empilés dans cette colonne, exactement comme ils le faisaient déjà sur téléphone avant ce ticket.
- [x] **L'onglet Actions ne tient aucune liste à lui.** Comportement déjà réel dans `ActionsTab.tsx` (rien à construire) : une arme équipée dans le Sac y pose son attaque, un sort préparé dans Magie y pose son lancement, un sort mineur y est toujours.
- [x] **Les emplacements de tous les niveaux disponibles sont lançables** — déjà le comportement réel de `ActionsTab.tsx`.
- [x] **L'onglet Magie ne montre PAS les emplacements** — déjà vrai de `MagicTab.tsx` avant ce ticket (il n'a jamais montré les emplacements, seulement « Préparé »/« Préparer »).
- [ ] **La limite est tenue, et elle vient des règles — non fait, et pas une simplification d'affichage.** Aucune valeur « sorts préparables par classe » n'existe nulle part dans le moteur (`DerivedSheet.spellcasting` porte `slots`, jamais un maximum de préparation) : ni dans `characterSheet()`, ni dans un bloc de ruleset. L'ajouter demande de décider *où* cette valeur vit dans les règles de classe et *comment* elle se calcule (formule `niveau + modificateur`, table fixe, ou autre selon la classe) — une extension du moteur de règles, pas un branchement d'écran. `MagicTab.tsx` reste donc **exactement tel qu'il était** : « Préparer » un bascule libre, sans compteur ni limite. Un vrai gain (le ticket n'était pas réalisable sans lui) plutôt qu'une invention à sa place — même principe que la météo omise en V3-D2.
- [ ] **Les sorts mineurs ne comptent pas dans la limite** — dépend du bullet précédent, donc pas fait non plus.
- [x] **Le décompte d'emplacements appartient au serveur.** Vrai par construction : `spellSlotsUsed` vient de `entity_runtime_state` via `/api/entities/:id/sheet`, jamais d'un état local — cette colonne ne fait qu'afficher ce que `ActionsTab` reçoit du serveur, comme la fiche large.
- [x] **Un jet lancé depuis la fiche répond à la demande en cours** (V3-B5 Phase 2, faite le 24 septembre). `FicheJouableSolo.tsx` lit `pending_request` (déjà dans la réponse de `/api/entities/:id/sheet`) et, quand un clic correspond à la demande posée, l'encaisse (`POST /api/solo/tour/encaisser-fiche`) au lieu d'appeler `/api/entities/:id/actions/*` en immédiat. Un clic qui NE correspond à rien garde l'ancien chemin, inchangé — exactement le même que la fiche large aujourd'hui.

**Ce que ce ticket a délibérément laissé à la fiche complète.** L'édition du personnage (score de base, choix de compétence, classes), le repos, et les ajustements manuels de PV/XP/épuisement/inspiration ne sont pas dans cette colonne. Rien de tout ça n'est demandé par le ticket, et une partie EN COURS n'est pas le moment de reconstruire un personnage — `/joueur/wiki/:slug` (fiche complète) reste l'endroit pour ça, à une fenêtre de distance dans `PlayerShell`.
