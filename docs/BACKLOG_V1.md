...
Blocs de prose (5.1 uniquement) : 32 fiches de règle, 8 `spellcasting_progression`, et désormais **280/296 monstres localisés** avec `traits`/`actions` en français (détail au point 7-8 ci-dessous). Encore à 0 côté 5.2.1 et pour les 16 monstres en échec de découpage.
...
7-8. ~~**Générique d'extraction des blocs `traits`/`actions` de monstre — conception, validation, exécution.**~~ **Fait le 13 août pour la SRD 5.1, en deux passes — 280/296 monstres localisés extraits avec succès (94,6 %).** `scripts/extract-monster-blocks-fr.ts` (`npm run extract:monster-blocks`, `--write` pour écrire ; `--only <entry_key>` + `DEBUG_ZONE=1` pour déboguer un seul monstre en conservant les bornes de zone de tous les autres, nécessaires à son propre calcul).
   ...
   **16 échecs restants, listés, jamais devinés** — chacun un cas distinct plutôt qu'un motif récurrent : `planetar`/`solar` ont un vrai trait manquant du texte français à cet endroit (`Divine Awareness`, absent, pas un bug de découpage) ; les autres (glabrezu, chain-devil, duergar, efreeti, gorgon, ghoul, lich, oni, pseudodragon, androsphinx, grimlock, swarm-of-insects, giant-rat, knight) restent à examiner individuellement. 20 monstres non localisés (nom homonyme non désambiguisable, ou 0 occurrence trouvée malgré un nom traduit — ex. nuées, nightmare, night-hag) — hors de portée de cette passe.
