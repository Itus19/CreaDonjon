...
- **Limite de données découverte, pas un bug de ce ticket** : les bundles espèce/historique n'ont qu'**une** carte chacun (pas une par trait racial individuel comme « Vision dans le noir » séparément) — le SRD ne découpe pas les traits raciaux en entrées `ruleset_entries` distinctes, seulement en un bloc `custom_table` unique. Distinction plus fine hors de portée sans réimport structurant.
- **Défaut de données révélé par ce ticket, signalé séparément (pas corrigé ici)** : `ai_digest` pour les entrées sans prose SRD (espèce, historique) vaut littéralement `"<nom> (<type>) — <nom>"` (`scripts/ingest-srd.ts:557`, repli `digestSource = prose ?? name` quand `prose` est `null`) — invisible tant que `chip.summary` ne s'affichait qu'en infobulle, maintenant visible en permanence dans l'encadré. Tâche de fond créée pour un futur ticket dédié plutôt que traitée ici (pas le sujet de V1-C9, qui ne fait qu'afficher ce qui est déjà résolu).
