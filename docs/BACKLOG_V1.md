...
Blocs de prose (5.1 uniquement) : 32 fiches de règle, 8 `spellcasting_progression`, et désormais **248/296 monstres localisés** avec `traits`/`actions` en français (détail au point 7-8 ci-dessous). Encore à 0 côté 5.2.1 et pour les 48 monstres en échec de découpage.
...
7-8. ~~**Générique d'extraction des blocs `traits`/`actions` de monstre — conception, validation, exécution.**~~ **Fait le 13 août pour la SRD 5.1 — 248/296 monstres localisés extraits avec succès.** `scripts/extract-monster-blocks-fr.ts` (`npm run extract:monster-blocks`, `--write` pour écrire, sans quoi rapport de couverture seul).

   **Méthode** : contrairement aux règles/objets, aucun catalogue de noms de trait n'existe pour vérifier un candidat à l'avance (334 monstres, jamais nommés individuellement) — le seul garde-fou possible est **le compte**. Pour chaque monstre déjà nommé en français (V1-A5/V1-D3b, 316/334), le script localise son en-tête dans le texte en désambiguïsant par CA + PV du bloc `stat_block` déjà importé (même technique que pour les 7 monstres retrouvés plus tôt cette session), puis détecte les en-têtes de trait/action par régularité structurelle (un en-tête commence toujours une ligne physique, majuscule initiale, se termine par un point suivi d'un espace, et n'est jamais précédé — sur la ligne d'avant — que par une ponctuation de fin de phrase, jamais un mot de liaison en fin de ligne comme « sa » ou « la »). Si le nombre d'en-têtes détectés ne correspond pas exactement au nombre de traits/actions attendu (`ruleset_entry_blocks`, déjà importé en anglais), le monstre est **ignoré et listé**, jamais deviné.

   **Trois bugs trouvés et corrigés avant d'écrire quoi que ce soit** :
   - Une mention en cours de phrase repliée en début de ligne physique par la mise en page (« ... recourir à sa \nPrésence terrifiante. Il effectue... ») se faisait passer pour un vrai en-tête — corrigé par la règle de ponctuation précédente ci-dessus, qui a fait passer le taux de réussite de 15/27 à 19/27 sur l'échantillon de validation.
   - **Bug silencieux, trouvé par relecture manuelle et non par le comptage** : quand un monstre non encore nommé s'intercale entre deux monstres localisés dans le texte, son contenu (nom, type, CA, PV...) se faisait absorber dans la description de la dernière entrée détectée — le compte restait pourtant correct, donc invisible sans relire le résultat. Repéré sur Hibou-ours → Hippogriffe et Mimique → Minotaure. Corrigé par une troncature de zone sur la deuxième occurrence de « de taille » (ligne type/gabarit, la plus proche du nom du monstre suivant) plutôt que d'attendre « Classe d'armure » ou l'en-tête de caractéristiques, cinq lignes plus loin. Un troisième repli sur le titre de section alphabétique (« Monstres (O) ») couvre le cas où le monstre intercalé n'a pas la ligne « de taille » standard (Objets animés, plusieurs gabarits dans une seule fiche).
   - Un filtre de rejet trop large (`Résistance` en préfixe nu, pour exclure « Résistance aux dégâts ») rejetait à tort de vrais traits comme « Résistance légendaire » — resserré à une expression précise (« Résistance(s) aux/(»).

   **Vérification avant écriture** : échantillon de 30 monstres inspecté à la main (dragon avec actions légendaires, golem, mort-vivant, humanoïde simple, bête), puis un contrôle automatique de dernier recours sur les 248 succès — recherche de mots de type de créature (Humanoïde, Dragon, Mort-vivant...) au *milieu* d'une description, signe d'une fuite non attrapée par les troncatures — **zéro trouvé**. `giant-rat-diseased` (la variante nichée dans le rat géant normal, déjà documentée V1-D3b point 5) génère volontairement deux « Morsure. » et est laissée hors de portée du générique.

   **48 échecs listés, jamais devinés** : essentiellement des dragons (souffle/attaques multiples avec des ponctuations internes qui déjouent la détection), des morts-vivants avec sous-titre parenthéré, et des monstres dont une phrase interne ressemble structurellement à un en-tête — chacun nommément listé en sortie du script pour une reprise manuelle ultérieure, pas masqué dans un compteur global. 20 monstres restent non localisés (nom homonyme non désambiguisable, ou 0 occurrence trouvée malgré un nom traduit — ex. nuées, nightmare, night-hag) — hors de portée de cette passe.
