...
1. **Espèces, historiques et règles — noms + prose officielle des règles.** Petit volume (15 noms manquants sur les deux rulesets, ~39 fiches de règle). La règle a une vraie prose officielle jamais extraite (contrairement au monstre, dont la description est une phrase synthétisée à l'import) — contenu dispersé dans tout le document plutôt que dans un chapitre unique (identifié par V1-A5, jamais traité), donc chaque section à délimiter à la main comme pour l'incantation de classe, pas un script générique.
   ...
   **Prose des règles — découverte qui change le chiffrage, pas encore commencée** : en lisant le chapitre « Utiliser les caractéristiques » pour amorcer l'extraction (`Bonus de maîtrise`/`Tests de caractéristique`/`Jets de sauvegarde`...), plusieurs sous-sections du texte officiel (`Opposition`, et probablement d'autres selon les chapitres) n'ont **aucune entrée `ruleset_entries` correspondante** — la borne « jusqu'au prochain nom de règle déjà connu » n'est donc pas sûre : une extraction naïve engloutirait ce contenu non suivi dans la fiche précédente, ou le couperait au milieu d'une idée. Contrairement à l'incantation de classe (bornes propres, sous-sections toutes connues à l'avance), le chapitre Règles a une profondeur de plan variable que V1-A5 avait déjà pressentie sans la mesurer (« sections de longueur très variable »). **Décision à prendre avant de continuer, pas à deviner** : soit (a) ne capturer que le paragraphe d'ouverture de chaque règle jusqu'à la première sous-section, quitte à laisser le détail dans `custom_table`/l'anglais, soit (b) capturer le chapitre entier jusqu'au prochain nom de règle connu, sous-sections non suivies comprises. Reporté à la prochaine session de ce point plutôt que tranché sans validation.
