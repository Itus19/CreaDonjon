...
- **Champs d'espèce/historique/classe restent des champs texte + suggestions**, pas convertis en listes déroulantes strictes — avis donné à l'utilisateur plutôt que tranché silencieusement : `RuleEntryAutocomplete` distingue déjà les entrées valides par suggestion tout en acceptant une clé libre (specs §B5, « avertir, ne pas interdire ») pour ne pas bloquer un MJ qui référence un contenu maison pas encore importé dans le ruleset. Un vrai menu déroulant perdrait cette marge de manœuvre. Le vrai problème signalé (« gagner de la place », « traduit directement ») est déjà résolu par le complément (5) : le nom résolu s'affiche en clair à côté du champ.
