...
7-8. ~~**Générique d'extraction des blocs `traits`/`actions` de monstre — conception, validation, exécution.**~~ **Fait le 13 août pour la SRD 5.1, en quatre passes — 321/334 monstres avec `traits`/`actions` en français (96,1 % du total), 334/334 avec un nom français.** `scripts/extract-monster-blocks-fr.ts` (`npm run extract:monster-blocks`, `--write` pour écrire ; `--only <entry_key>` + `DEBUG_ZONE=1` pour déboguer un seul monstre en conservant les bornes de zone de tous les autres, nécessaires à son propre calcul).
