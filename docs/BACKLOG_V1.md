...
| Espèce | 13/13 | 24/39 |
| Historique | 1/1 | 4/4 |
| Règle | 36/39 (31 avec prose) | 28/39 |
...
Blocs de prose (`traits`/`actions`/description de règle) : encore à 0 hors des 31 fiches de règle et 8 `spellcasting_progression` déjà faits (5.1 uniquement).
...
1. **Espèces, historiques et règles — noms + prose officielle des règles.** Petit volume (15 noms manquants sur les deux rulesets, ~39 fiches de règle). La règle a une vraie prose officielle jamais extraite (contrairement au monstre, dont la description est une phrase synthétisée à l'import) — contenu dispersé dans tout le document plutôt que dans un chapitre unique (identifié par V1-A5, jamais traité), donc chaque section à délimiter à la main comme pour l'incantation de classe, pas un script générique.
   ...
   **Prose des règles — trois groupes faits, 31 fiches, choix produit tranché par l'utilisateur (chapitre entier).** `scripts/translate-rules-fr.ts` (`npm run translate:rules`) couvre désormais :
   - **Groupe 1** : « Utiliser les caractéristiques » → « Le passage du temps » → « Aventure » → « Combat » → « Incantation » (avantage/désavantage, bonus de maîtrise, tests de caractéristique, jets de sauvegarde, le passage du temps, déplacement, environnement, repos, entre les aventures, ordre du combat, déplacement et position, actions au combat, utiliser un objet, effectuer une attaque, dégâts et soins, combat monté, combat subaquatique, qu'est-ce qu'un sort, lancer un sort).
   ...
   - Reste pour une prochaine session de ce même point : le reste du chapitre Équipement (tables d'armes/armures/objets d'aventurier, catalogue déjà couvert par les blocs V1-D1/D2, valeur ajoutée faible), Caractéristiques réel (l'occurrence trouvée à la ligne 25795 s'est révélée appartenir à Objets magiques intelligents, pas à `ability-scores-and-modifiers` — la vraie fiche reste à localiser), et l'intégralité de la SRD 5.2.1 (aucune prose de règle traduite côté 2024 pour l'instant).
