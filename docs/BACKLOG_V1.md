...
7-8. ~~**Générique d'extraction des blocs `traits`/`actions` de monstre — conception, validation, exécution.**~~ **Fait le 13 août pour la SRD 5.1, en trois passes — 303/334 monstres avec `traits`/`actions` en français (90,7 % du total).** `scripts/extract-monster-blocks-fr.ts` (`npm run extract:monster-blocks`, `--write` pour écrire ; `--only <entry_key>` + `DEBUG_ZONE=1` pour déboguer un seul monstre en conservant les bornes de zone de tous les autres, nécessaires à son propre calcul).
   ...
   **13 échecs restants, listés, jamais devinés**, chacun un cas distinct plutôt qu'un motif récurrent : `planetar`/`solar` ont un vrai trait manquant du texte français à cet endroit (`Divine Awareness`, absent, pas un bug de découpage) ; les autres (glabrezu, chain-devil, duergar, efreeti, night-hag, lich, oni, pseudodragon, androsphinx, grimlock, knight) restent à examiner individuellement. 18 monstres n'ont encore aucun nom français (`entry_key` hors des 316 déjà traités) — hors de portée de cette passe, condition préalable à leur localisation.
