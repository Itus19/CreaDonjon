...
- [ ] Traduire Objet et Aptitude en tenant compte de la nouvelle structure post-V1-A2 (Objet 30 % → **57 %**, Aptitude 28 % → **62 %** — catégories les plus volumineuses, 1169 et 961 entrées, en net progrès mais pas finies).
...
- **Règle (78 entrées, la plus petite catégorie)** : Actions in Combat = **Actions au combat**, Between Adventures = **Entre les aventures**, Diseases = **Maladies** confirmés et écrits — mais la couverture de NOMS ne bouge pas (toujours 73 %), car les noms encore manquants appartiennent tous à la ruleset 2024, dont plusieurs chapitres (Adventuring, The Order of Combat, The Environment, The Planes of Existence, Using Each Ability, What Is a Spell?, Appendix, Standard Exchange Rates, Sentient Magic Items) sont soit restructurés différemment soit introuvables comme en-tête isolé dans `srd-5.2.1-fr.txt` — pas une erreur de méthode, une vraie différence de plan entre les deux éditions.
