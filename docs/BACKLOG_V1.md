...
| Espèce | 13/13 | 24/39 |
| Historique | 1/1 | 4/4 |
| Règle | 34/39 (26 avec prose) | 28/39 |
...
Blocs de prose (`traits`/`actions`/description de règle) : encore à 0 hors des 26 fiches de règle et 8 `spellcasting_progression` déjà faits (5.1 uniquement).
...
1. **Espèces, historiques et règles — noms + prose officielle des règles.** Petit volume (15 noms manquants sur les deux rulesets, ~39 fiches de règle). La règle a une vraie prose officielle jamais extraite (contrairement au monstre, dont la description est une phrase synthétisée à l'import) — contenu dispersé dans tout le document plutôt que dans un chapitre unique (identifié par V1-A5, jamais traité), donc chaque section à délimiter à la main comme pour l'incantation de classe, pas un script générique.
   ...
   **Prose des règles — deux groupes contigus faits, 26 fiches, choix produit tranché par l'utilisateur (chapitre entier).** `scripts/translate-rules-fr.ts` (`npm run translate:rules`) couvre désormais :
   ...
   - **Quatre erreurs de borne trouvées et corrigées avant écriture définitive**, toutes reconnues par le même motif — un paragraphe d'introduction de chapitre **sans en-tête propre** qui s'intercale entre deux fiches suivies en base et se fait absorber à tort par celle qui précède : la borne haute de `casting-a-spell` avait d'abord avalé toute l'annexe « Listes de sorts » par classe (lignes 9873-10753, 28987 caractères au lieu de 14655) ; `saving-throws` avait avalé l'intro « Le passage du temps », qui s'est révélée être un vrai nom de règle jamais trouvé jusque-là (`time`, en base sous un nom différent de l'en-tête réel — recherché comme « Temps » seul, jamais comme « Le passage du temps ») ; `underwater-combat` avait avalé l'intro sans titre du chapitre Incantation (« Sorts ») ; `poisons` avait avalé l'intro sans titre du chapitre Objets magiques. Les trois derniers cas : contenu simplement exclu des deux fiches voisines plutôt que mal attribué à l'une d'elles — repérés en relisant systématiquement la fin de **chaque** texte extrait avant d'écrire en base, pas seulement les cas qui semblaient suspects.
   - Toutes les 26 fiches relues en fin de texte extrait après corrections : contenu cohérent avec le sujet de la fiche jusqu'à la toute dernière phrase, aucune anomalie restante.
   - Reste pour une prochaine session de ce même point : les fiches de règle isolées plus loin dans le document 5.1 (Caractéristiques vers la ligne 25795, Les plans d'existence vers 36242) et l'intégralité de la SRD 5.2.1 (aucune prose de règle traduite côté 2024 pour l'instant).
