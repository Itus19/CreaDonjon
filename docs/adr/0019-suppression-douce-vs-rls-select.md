...
## Corollaire trouvé en testant le rétablissement (retour utilisateur : "teste aussi le rétablissement d'une fiche qui a des blocs")

Toute lecture applicative qui doit voir une ligne masquée par `entities_select` — pas seulement l'écriture — a le même problème. `generateUniqueEntitySlug` (création de fiche) vérifie l'unicité d'un slug via `worldHasSlug`, un `.from("entities")` normal sous RLS : une fiche supprimée reste invisible à cette vérification alors qu'elle occupe TOUJOURS son slug (la contrainte `entities_world_id_slug_key` ne filtre jamais `deleted_at` — nécessaire, sinon un rétablissement échouerait pour la raison inverse). Résultat concret, reproduit en testant ce rétablissement : créer une fiche pouvait échouer en brut 500 "duplicate key value violates unique constraint", le contrôle d'unicité ayant cru le slug libre.

Corrigé par le même patron exactement : `public.world_has_slug(p_world_id, p_slug)`, `security definer set row_security = off`, vérifie l'appartenance au monde puis l'existence SANS filtre `deleted_at`. Généralise la leçon de cet ADR au-delà des écritures : **toute vérification (lecture OU écriture) qui doit tenir compte d'une ligne masquée par une politique SELECT liée à `deleted_at` a besoin du même contournement** — pas seulement l'UPDATE qui pose `deleted_at` lui-même.
