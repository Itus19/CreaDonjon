...
- [ ] **Reporté à une prochaine session** : mode "référence" du bloc `map` (Phase F₁ ci-dessous) — c'est lui qui permettra de "mettre n'importe quelle fiche carte dans les blocs de carte des autres fiches" (ex. le bloc `map` de Fine Lââm référençant la fiche `carte` "Faerûn"). Le bloc `map` posé sur une autre fiche reste pour l'instant utilisable seulement en mode "own" (sa propre image, indépendante).
