...
**Bug repéré en passant, hors périmètre — corrigé le 19 septembre** : `RandomTableBlockEditor.tsx`, bouton « + Ajouter un résultat » — la nouvelle entrée héritait d'une plage de largeur nulle (`min = max = ancien_max + 1`) au lieu de s'étendre jusqu'au nombre de faces du dé, ce qui rendait la table invalide (aucune entrée ne couvre les résultats intermédiaires) tant qu'on ne corrigeait pas la plage à la main. Rencontré et contourné à répétition en saisissant le contenu Taverne.

Le calcul est sorti du composant vers `nextEntryRange` (`src/core/tables/roll.ts`, 5 tests écrits avant la correction) : une entrée ajoutée prend désormais tout ce qui RESTE du dé, donc plus jamais de trou et plus de `NoMatchingEntryError` au tirage. Deux cas limites tranchés explicitement plutôt que devinés — un dé en cours de frappe (« d » avant « d20 ») retombe sur l'ancien comportement de largeur 1, et une dernière entrée couvrant déjà tout le dé (**l'état de départ d'un bloc neuf**, `registry.ts` pose `{1,20}` sur un d20) donne une entrée hors dé : inatteignable au tirage, mais jamais un trou. Ce second cas n'est donc **pas** réglé par cette correction — répartir les plages d'une table pleine reste un geste d'auteur, et rétrécir silencieusement l'entrée existante pour faire de la place aurait été plus surprenant que le laisser visible.

**Vérifié en direct** (monde ClaudeLand, serveur de dev) : bloc Table aléatoire neuf → entrée unique `1–20` sur un d20, confirmant que l'état de départ est bien une table pleine ; première entrée rétrécie à `1–5` puis « + Ajouter un résultat » → la nouvelle plage est `6–20` (elle aurait été `6–6` avant) ; 20 tirages sur l'état persisté → 5 « Entree A » / 15 « Entree B », aucune erreur. **Contre-épreuve** : plage de la seconde entrée ramenée à `6–6` à la main pour recréer le trou, rechargement pour être sûr de tirer sur l'état persisté, puis tirage → « Aucune entrée de table ne couvre le résultat 20 » (HTTP 400 sur `/draw`). Le test précédent était donc bien sensible : sans la correction, il aurait échoué. Dé incomplet (`d`) puis ajout → plage `7–7`, largeur 1, aucun plantage — et le PATCH est refusé par Zod (`^d\d+$`), la validation faisant son travail. Fiche de test supprimée après vérification.

**Piège rencontré en vérifiant** : les premiers tirages renvoyaient « Nouveau résultat » et non les textes saisis. `POST /api/blocks/[blockId]/draw` tire sur la version **persistée**, pas sur l'état de l'éditeur : un tirage lancé avant que l'enregistrement ait abouti mesure l'ancienne table. Toute vérification de tirage doit passer par un rechargement préalable, sinon elle ne prouve rien.
...
**Tranché le 24 septembre, par l'auteur : (a), avec l'origine affichée.** La recommandation (b) ne tient pas avec l'architecture : un monde n'a qu'**un** ruleset actif (`worlds.default_ruleset_id`), et un ruleset `personal_reference` ne peut dériver que d'une base officielle (`createRulesetVariant`) — il ne peut donc pas se glisser sous la variante active, et une fiche écrite dans une telle « couche » n'apparaîtrait pas dans le monde. Le formulaire écrit dans la variante active, mais dit laquelle **et son origine** (« référence personnelle : n'y saisis que la mécanique » ou « variante maison ») : le choix n'est jamais implicite. Qui veut saisir du contenu de livre active d'abord un ruleset `personal_reference`. Rouvrir (b) demanderait de décider comment deux rulesets coexistent dans un monde — un ADR, pas un formulaire.

### V2-N1 — Créer une sous-classe · `M` — **fait le 24 septembre, non vérifié en direct**
...
- [x] Champs : nom, classe parente (menu déroulant des classes du ruleset actif), description **courte** (un champ d'une ligne, 2 000 caractères au plus, validé par Zod) + `page_ref`, liste répétable d'aptitudes (nom, niveau 1 à 20, quelques mots). Formulaire `CreateHomebrewSubclassForm.tsx`, page `/regles/nouvelle-sous-classe`, fenêtre flottante, entrée « Créer une sous-classe » du menu « Ajouter une règle ». **Le texte d'aide** reprend mot pour mot la première phrase de celui du don (« Recopie les valeurs depuis ta propre source (le contenu n'est jamais deviné ici). ») — c'est la seule qui soit un garde-fou : le reste de l'aide du don parle d'effets chiffrés, et son champ de description invite au contraire à « recopier le texte de ta source ». Elle est complétée d'une phrase qui envoie vers la référence de page.
- [x] Écrit par `createHomebrewSubclass` (`src/server/services/rules.ts`), qui réutilise `importRulesetEntries` : `add_entry` (`entry_type: "subclass"`, `parent_class_key`) puis un `add_block` par bloc rempli. Un champ vide ne produit jamais de bloc à moitié rempli : pas de bloc `description` si ni texte ni page, pas de `page_ref: ""`, une aptitude sans nom est écartée (`buildHomebrewSubclassEntry`, `src/core/rules/homebrewSubclass.ts`, testé d'abord). Route `POST /api/rulesets/[rulesetId]/subclasses`, validée par Zod ; au moins une aptitude nommée exigée.
- [x] Patch du `subclass_slot` de la classe choisie : le tableau `options[]` est relu **résolu** (base + toutes les surcharges) puis réécrit en entier. Et un second piège, trouvé en l'écrivant : la variante n'a qu'**une** ligne de surcharge par `(entry_key, block_type)`, donc l'upsert **remplace** ce qui était déjà posé à ce niveau. Si c'était un patch, ses autres champs sont repris ; si c'était un bloc ajouté ou remplacé, il le reste (`subclassSlotWrite`, testé contre le vrai résolveur). Une classe sans `subclass_slot` n'en reçoit pas, et l'écran le dit. **Correction du constat** : ce patch ne rend pas la sous-classe « sélectionnable » — l'assistant de création et le menu de la fiche filtrent déjà par `parent_class_key` (V2-J5), sans lire `options`. Le patch sert la liste des sous-classes affichée sur la page de la classe. En revanche, une classe **sans** `subclass_slot` ne propose aucune sous-classe à la création, quelle qu'elle soit.
- [ ] Cas où la sous-classe a elle-même une progression de sorts propre (ex. Arnaqueur arcanique, Lame occulte) : `custom_table` libre en échappatoire, pas un bloc dédié — aucune des 12 sous-classes officielles n'en a besoin (leur incantation vient entièrement de la classe), donc pas de bloc typé à inventer pour ce seul cas tant qu'un deuxième cas concret ne le réclame pas (règle des trois). **Pas dans le formulaire** : il ne propose pas de `custom_table`. L'échappatoire qui existe aujourd'hui : saisir **toute** la sous-classe par l'import JSON (« Règles actives »), qui accepte un bloc `custom_table` — pas la compléter après coup, l'import refuse une clé déjà présente, et aucun écran n'ajoute un bloc à une fiche maison existante. À ajouter au formulaire si ça gêne réellement.
- [ ] Nichage sidebar vérifié : la sous-classe créée apparaît sous sa classe parente, pas dans un groupe à part. **Vrai par construction** (`parent_class_key` → `parentClassKey` → `subclassesByParent`, `RulesSidebar.tsx`, déjà en place depuis V2-J5), mais **pas vérifié en direct** : la session cloud n'avait pas de base. À faire chez l'auteur, avec la page de la classe et l'étape Classe de l'assistant.

### V2-N2 — Créer un sort · `L` — **fait le 24 septembre, non vérifié en direct**
...
Même décision de cible que V2-N1 : la variante active, son origine affichée. Formulaire `CreateHomebrewSpellForm.tsx`, page `/regles/nouveau-sort`, fenêtre flottante, entrée « Créer un sort » du menu « Ajouter une règle ». La fiche est construite par `buildHomebrewSpellEntry` (`src/core/rules/homebrewSpell.ts`, testé d'abord) dans la **forme exacte de l'import SRD** (`spellBlocks`, `scripts/ingest-srd.ts`), puis écrite par `createHomebrewSpell` → `importRulesetEntries`. Route `POST /api/rulesets/[rulesetId]/spells`, validée par Zod.

- [x] Champs `spell_casting` (toujours) : nom, niveau (0-9, « Tour de magie » pour 0), école (menu des 8, clés anglaises du SRD, libellés de `MAGIC_SCHOOL_LABELS_FR`), temps d'incantation, portée, composantes (cases V/S/M, texte libre si M coché — omis sinon), durée, concentration, rituel. Plus une description **courte** et une référence de page, optionnelles, comme V2-N1.
- [x] Section « Effet chiffré » **optionnelle, fermée par défaut** (un bouton l'ouvre, un autre la retire), même ton que le formulaire de don : type d'effet (sauvegarde / jet d'attaque / aucun), caractéristique de sauvegarde, portée d'attaque (corps à corps / distance — le champ `attack.range` qu'exige le bloc), formule de dégâts, type de dégâts, effet en cas de réussite (`none`/`half`/`other`, les trois valeurs du SRD). **Sans effet, aucun bloc `effects`** — comme les quatre sorts sur cinq du SRD qui n'en ont pas. Conséquence à connaître : `REQUIRED_BLOCKS` exige `effects` pour un sort, donc la fiche affiche l'avertissement de bloc manquant, **exactement comme Détection de la magie** aujourd'hui. Écrire un bloc `effects` vide pour le faire taire aurait été un bloc à moitié rempli ; c'est plutôt `REQUIRED_BLOCKS` qui est trop strict pour les sorts, à revoir à part. **Un piège trouvé en testant** : le parseur de formules accepte une référence nommée, donc « beaucoup » passait comme une variable et n'aurait planté qu'au premier lancement. Les formules d'un sort maison n'acceptent que des dés et des nombres, refusées à la saisie avec un message.
- [x] Section « Montée en puissance » optionnelle, visible seulement si un effet chiffré est renseigné : table libre palier → nouvelle formule (bloc `scaling`, forme `table`, `rule: null`). **Écart assumé avec la lettre du ticket**, qui dit « niveau de personnage » : c'est vrai d'un tour de magie, pas d'un sort à emplacement. L'axe suit donc le SRD — `character_level` (palier de base 1) pour un tour de magie, `slot_level` (palier de base = niveau du sort) sinon — et le texte d'aide le dit. Le palier de base est écrit dans la table à partir de la formule de l'effet ; un palier inférieur ou égal est refusé. Vérifié par test : la table écrite est bien celle que `resolveScaledFormulaText` (le lancement réel, `castSpell`) relit.
- [x] Classes autorisées : une case par classe du ruleset actif, au moins une exigée. Écrit un bloc `custom_table` (`field: "classes"`/`field: "level"`, même forme brute que l'import SRD, replié, libellé « Classes et niveau ») — **la voie courte**, sans toucher `srdMapping.ts` : un test relit le bloc écrit avec `parseSpellClasses`/`parseSpellLevel`, les deux fonctions qu'utilise `SpellSelectionStep.tsx`. Les classes partent du client en **clés** seulement ; le serveur vérifie que chacune est une classe de la chaîne et relit lui-même son nom.
- [ ] Vérifié en direct : un sort créé pour une classe apparaît bien proposé à l'étape Sorts de l'assistant de création de personnage pour cette classe, à son niveau. **Pas fait** : la session cloud n'avait pas de base. Vrai par construction et par test de la forme écrite, mais c'est précisément le critère qui dit « en direct ». À faire chez l'auteur — avec, au passage, un lancement depuis l'onglet Actions d'un sort à effet chiffré.
