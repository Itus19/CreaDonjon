...
- [x] Réorganiser les blocs par glisser-déposer (V2-G1, `@dnd-kit`, poignée `⠿` — accessible au clavier et au tactile, pas seulement à la souris ; persiste `display_order` via la même route `PATCH /api/blocks/[id]/order` que les boutons Monter/Descendre, conservés en repli)
...
- [x] Compendium (Phase 1) : onglet "Règles" dans la barre latérale (bascule avec "Fiches"), catégories regroupant les `entry_type` bruts (Sorts/Classes/Espèces/Origines/Monstres/Armes/Armures/Outils/Objets/Dons/Traits/Compétences/Référence — "features"/"levels"/"equipment-categories" volontairement exclus, ce sont des tables de progression ou des pages d'index, pas des entrées consultables), sélecteur de ruleset 2014/2024, recherche par nom, fenêtre de détail en lecture seule avec rendu générique clé/valeur. Découverte au passage : `human_readable.desc` est de forme incohérente selon les entrées (array, string ou null) — normalisé côté client, mais bon signal que `structured_data` mériterait des rendus dédiés par type plutôt que le fallback générique (Phase 2, voir plus haut)
  ...
  - [x] Traduction des noms/descriptions à partir d'une vraie source : `dnd5eapi.co` (même origine que nos SRD) accepte `?lang=fr-FR` et renvoie du contenu français réel là où une traduction communautaire existe, avec repli anglais automatique sinon. Script `scripts/fetch-fr-translations.mjs` (rejouable), exécuté le 2026-07-28 : 4011 entrées vérifiées, 1337 mises à jour avec du vrai français. Couverture confirmée très inégale — bien couverts : sorts, aptitudes de classe (`features`), sections de règles, maîtrises, traits, états ; quasiment pas couverts côté source elle-même : monstres, objets/objets magiques, dons, alignements, origines, scores de caractéristiques, poisons, espèces. Pas un choix de notre part, l'API renvoie déjà l'anglais pour ces catégories-là — rejouer le script plus tard captera les traductions ajoutées côté `5e-bits/5e-database`
  - [ ] Constat : le SRD 2024 importé (`srd-2024.json`) ne contenait aucun sort (`Spells` absent du fichier source lui-même) ni sections de règles générales — comblé le 2026-07-28 en copiant ce contenu depuis 2014 (2024 révise 2014, ne le remplace pas) ; races/subraces volontairement non copiées, `species`/`subspecies` couvre déjà ce terrain en 2024
