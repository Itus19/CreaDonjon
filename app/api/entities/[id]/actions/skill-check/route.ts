...
import { skillCheckSchema } from "@/lib/characterActions/schemas";
import { rollSkillCheck } from "@/src/server/services/checkRolls";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = skillCheckSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const result = await rollSkillCheck(supabase, {
    entityId,
