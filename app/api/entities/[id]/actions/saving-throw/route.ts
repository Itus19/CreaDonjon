...
import { savingThrowSchema } from "@/lib/characterActions/schemas";
import { rollSavingThrow } from "@/src/server/services/checkRolls";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = savingThrowSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const result = await rollSavingThrow(supabase, {
    entityId,
