...
import { abilityCheckSchema } from "@/lib/characterActions/schemas";
import { rollAbilityCheck } from "@/src/server/services/checkRolls";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = abilityCheckSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const result = await rollAbilityCheck(supabase, {
    entityId,
