...
import { createMapPinSchema } from "@/lib/mapPins/schemas";
import { getEntityById } from "@/src/server/repos/entities";
import { getBlockById } from "@/src/server/repos/blocks";
import { buildViewerForWorld } from "@/src/server/services/visibility";
import { createMapPin, listVisibleMapPins } from "@/src/server/services/mapPins";

/** Punaises d'un bloc `map` propriétaire (Lot I, phase C) — `blockId` est toujours le bloc SOURCE (jamais un bloc "ref", qui n'en porte aucune, voir ADR 0017). */
export async function GET(_request: NextRequest, { params }: { params: Promise<{ blockId: string }> }) {
  ...
  const pins = await listVisibleMapPins(supabase, blockId, viewer);
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ blockId: string }> }) {
  ...
  const parsed = createMapPinSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const result = await createMapPin(supabase, {
    blockId,
    x: parsed.data.x,
    y: parsed.data.y,
  ...
  return NextResponse.json(result.pin, { status: 201 });
}
