 ...
 *
 * Sur son propre sous-chemin (`/mj/gestion-campagne`), pas sur la racine
 * `/mj` (retour utilisateur : "les boutons rouges pour fermer les fenetres
 * MJ ne fonctionnent pas") — bug reel trouve en verifiant : fermer une
 * fenetre primaire navigue vers `sectionHomeHref(worldSlug, "mj")`, qui
 * valait `/mj` ; tant que cet outil vivait justement sur `/mj`, "fermer"
 * navigait vers lui-meme, donc rien ne changeait a l'ecran. `/mj` est
 * desormais un accueil neutre (`app/m/[worldSlug]/mj/page.tsx`), sans
 * fenetre primaire — meme motif que Monde/Regles, qui ont deja chacun un
 * accueil sans fiche ouverte.
 */
  ...
  return (
    <div className="flex flex-col gap-4">
      <RegisterPrimaryWindow
        windowRef={{ kind: "mj", key: "gestion-campagne" }}
        name="Gestion de campagne"
        badge=""
        homeHref={`/m/${worldSlug}/mj`}
      />
      <CampaignsPanel
        worldSlug={worldSlug}
